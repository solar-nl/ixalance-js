...
import { FPU, FPU_REVISION } from './fpu.js';

// Re-exported so worker.js can require the exact FPU contract the interpreter constructed.
// A stale cached cpu.js lacks this export and fails at module linking instead of silently
// running with pre-fix float-to-integer semantics.
export { FPU_REVISION };
...
// Direct-mapped, tag-compared cache. A miss or a stale span generation just recompiles,
// so eviction needs no bookkeeping at all. 128 K slots covers a demo image without
// aliasing; the byte cap keeps a block inside two generation spans, which is what makes
// invalidation a two-generation compare.
const BLOCK_SLOTS = 1 << 17, BLOCK_MASK = BLOCK_SLOTS - 1;
const BLOCK_MAX_OPS = 64, BLOCK_MAX_BYTES = 224;
const BLOCK_GEN_SHIFT = 8, BLOCK_GEN_BYTES = 1 << BLOCK_GEN_SHIFT;
// compile() explicitly refuses to add an instruction that would make a block longer than
// one 256-byte generation span. Every block can therefore still be validated by its two
// endpoint generations. Stash's
// generated rasterizers patch code densely: the old 4 KiB span made one real patch force
// byte-identical redecodes of unrelated blocks on the same page tens of millions of times.
...
// Shifts are the fixed-point rasterizers' hottest generic instructions. The arithmetic
// stays owned by shift(); this handler removes only opcode dispatch, immediate fetch and
// ModRM decoding, exactly like the x87 handlers above.
function h_shift(c, o) {
  const m = c._m;
  m.mod = o.mod; m.reg = o.reg; m.rm = o.rm; m.raw = o.raw;
  m.addr = o.mod === 3 ? null : ea(c, o);
  const size = o.id === 0xc0 || o.id === 0xd0 || o.id === 0xd2 ? 1 : o.opsize;
  const amount = o.id === 0xc0 || o.id === 0xc1 ? o.imm & 31
    : o.id === 0xd0 || o.id === 0xd1 ? 1 : c.get8(1) & 31;
  c.shift(m, size, amount);
}
...
function pick(o) {
  ...
  if (id === 0xc0 || id === 0xc1 || (id >= 0xd0 && id <= 0xd3)) return h_shift;
...
export class CPU {
  constructor(machine) {
    ...
    // rawMem is instruction fetch/decode and internal JIT bookkeeping. mem is the guest
    // data view and can be replaced by TracedDataView while the Pandora/scene inspector is
    // active. In ordinary playback both are the exact same DataView.
    this.rawMem = machine.mem;
    ...
    this.pageGen = new Uint32Array((machine.size >> BLOCK_GEN_SHIFT) + 2);
    this.codeDirty = 0;
  }
   ...
   * images put code and data close together, and at coarse granularity the demo's own
   * generated-code writes invalidate unrelated cached blocks.
   *
  ...
  codeTouch(addr, size) {
    const p = addr >>> BLOCK_GEN_SHIFT;
    const q = (addr + size - 1) >>> BLOCK_GEN_SHIFT;
  ...
  fetch8() { return this.rawMem.getUint8(this.eip++); }
  fetch16() { const v = this.rawMem.getUint16(this.eip, true); this.eip += 2; return v; }
  fetch32() { const v = this.rawMem.getUint32(this.eip, true) >>> 0; this.eip += 4; return v; }
  ...
  context() {
    const bytes = [];
    for (let i = 0; i < 12; i++) {
      try { bytes.push(this.rawMem.getUint8(this.insStart + i)); } catch { break; }
    }
    return {
      eip: this.insStart >>> 0,
  ...
  compile(eip, slot) {
    ...
    for (let i = 0; i < BLOCK_MAX_OPS; i++) {
      ...
      if (o === null) break;
      if (p + o.len - eip > BLOCK_GEN_BYTES) break;
      ops.push(o);
      p = (p + o.len) >>> 0;
    ...
    const p0 = eip >>> BLOCK_GEN_SHIFT;
    const p1 = ops.length === 0 ? p0 : (p - 1) >>> BLOCK_GEN_SHIFT;
  ...
  decodeOne(p) {
    ...
    switch (d >>> 3) {
      ...
      case I_G3:
        // Only /0 and /1 take an immediate. Keep its bits in the record just like the
        // other immediate classes so compiled consumers need not re-read instruction bytes.
        if (reg < 2) {
          imm = id === 0xf6 ? u8[p]
            : opsize === 2 ? (u8[p] | (u8[p + 1] << 8)) : rd32le(u8, p);
          p += id === 0xf6 ? 1 : opsize;
        }
        break;
    }
  ...
  step() {
    ...
    for (;;) {
      const b = this.rawMem.getUint8(this.eip);

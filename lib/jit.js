...
// generation-span bump. With the original 4 KB spans, jizz bumped generations 1.24 M
// times per 200 M instructions
...
//
// STAGE 11 — REUSE DECODED BLOCKS BEFORE REVALIDATION. Stash's generated rasterizers made
// CPU.block() decode a block again before jitFor() could prove its old bytes were identical.
// A compiled entry now retains the decoded record it was built from. jitDirect() checks the
// entry's generation first, then its byte snapshot, and only genuine byte changes reach the
// existing decoder/version/shape path. On BLUISH order 3 rows 32-64, 191.9M of 195.6M
// compiled-block dispatches reuse their record directly, including 12.1M stale-generation
// byte proofs. Genuine variants and every interpreter fallback remain on the old path.
//
// ---------------------------------------------------------------------------------
...
export const JIT_REVISION = 'provenance-context-v3';
 ...
 * hot path.
 */
function blockSrc(b, live = false, trace = false) {
  ...
  for (let k = 0; k < n; k++) {
    ...
    const context = trace
      ? `c.insStart = ${hexu(ops[k].at)}; c.count = base + ${k + 1};\n  `
      : '';
    body.push(eager ? `  i = ${k};\n  ${context}${s}` : `  ${context}${s}`);
  }
...
export class JitCPU extends CPU {
  constructor(machine) {
    ...
    // every generation-span bump (1.24 M per 200 M instructions in the original profile)
    ...
    this.jitDirectHits = 0;    // saved decoded blocks reused before CPU.block()
    this.jitDirectVerify = 0;  // direct hits that first proved byte identity
  ...
  /**
   * Reuse a compiled entry's decoded block before CPU.block() throws it away on a stale
   * generation. Equal generations are the ordinary write-barrier proof. If an unrelated
   * write changed one of those generations, equal saved bytes are the stronger proof:
   * decodeOne() is a pure function of the bytes, so the retained records still describe
   * the current instruction stream exactly. A genuine byte change returns null and takes
   * the existing decode/version/shape path.
   *
   * The direct-mapped slot is deliberately the only lookup here. A rare collision falls
   * back to block()+jitFor(), which restores the slot from the authoritative Map without
   * putting a Map lookup on every block dispatch.
   */
  jitDirect(eip) {
    this.codeDirty = 0;
    const slot = (eip ^ (eip >>> 15)) & JIT_SLOT_MASK;
    const e = this.jitSlot[slot];
    if (e === null || e.start !== eip || e.poisoned || e.fn === null || e.block === null) {
      return null;
    }
    if (e.g0 === this.pageGen[e.p0] && e.g1 === this.pageGen[e.p1]) {
    ...
    if (STATS) {
      this.jitReverify++;
      this.jitDirectHits++;
      this.jitDirectVerify++;
    }
    return e;
  }

  /**
   * The compiled function for a newly decoded block, or null to stay on the interpreter
   * tier. jitDirect() handles the common retained-block path before this is reached.
   *
  ...
  jitRefreshDynamic(e) {
    ...
    // This is JIT bookkeeping reading instruction bytes, not a guest data access.
    const fields = e.liveFields, ops = e.block.ops, mem = this.rawMem;
   ...
   *
   * Spans average 14-30 bytes and BLOCK_MAX_BYTES caps them at 224, so the memcmp is
  ...
  jitEmit(b, e, hash, live = false) {
    ...
    const { src, mods } = blockSrc(b, live, this.mem !== this.rawMem);
  ...
  jitStatsAtExit() {
    ...
    p.on('exit', () => {
      ...
      p.stderr.write(
        ...
        + `jit: direct=${this.jitDirectHits} byteVerified=${this.jitDirectVerify}\n`
  ...
  run(budget = 1e7) {
    ...
    try {
      while (remaining > 0 && !this.halted) {
        let e = this.jitDirect(this.eip);
        const b = e === null ? this.block(this.eip) : e.block;
        if (b !== null && b.len <= remaining) {
          if (e === null) e = this.jitFor(b);
          if (e !== null) {

...
export class XmPlayer {
  ...
  parse(b) {
    ...
    for (let i = 0; i < numInstruments; i++) {
      ...
      const inst = { samples: [], keymap: new Uint8Array(96) };

      if (numSamples > 0) {
        const shdrSize = u32(b, at + 29);
        inst.keymap.set(b.subarray(at + 33, at + 33 + 96));
        // Envelopes exist in the format but none of these modules enable them.
        if (b[at + 0x10d] & 1) this.unsupported.add('volume envelope');
        if (b[at + 0x10e] & 1) this.unsupported.add('panning envelope');
  ...
  startRow() {
    ...
    for (let i = 0; i < this.channels; i++) {
      ...
      if (fx === 0xe && (param >> 4) === 0xd && (param & 15) > 0) {
        ...
        c.playing = false;                                  // key off, no envelopes
  ...
  mix(left, right, offset, count) {
    ...
    for (const c of this.ch) {
      if (!c.playing || !c.sample || c.volume <= 0) continue;
      const s = c.sample;
      ...
      const vol = (Math.min(64, c.volume) / 64) * gv * 0.22;   // headroom for 14-16 channels

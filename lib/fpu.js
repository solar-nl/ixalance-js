...
export class FPU {
  ...
  /**
   * Execute one x87 instruction. `op` is the primary opcode (0xd8..0xdf); the ModRM
   * byte has not been consumed yet.
   */
  execute(op) {
    const cpu = this.cpu;
    const m = cpu.modrm();
      ...
      case 0xd9: {
        if (isMem) {
          switch (reg) {
            ...
            case 2: cpu.mem.setFloat32(m.addr, this.get(0), true); return;    // fst m32
            case 3: cpu.mem.setFloat32(m.addr, this.pop(), true); return;     // fstp m32
            case 5: this.cw = cpu.mem.getUint16(m.addr, true); return;        // fldcw
            case 7: cpu.mem.setUint16(m.addr, this.cw, true); return;         // fnstcw
      ...
      case 0xdb: {
        if (isMem) {
          switch (reg) {
            ...
            case 1: cpu.mem.setInt32(m.addr, this.round(this.get(0)) | 0, true); this.pop(); return; // fisttp
            case 2: cpu.mem.setInt32(m.addr, this.round(this.get(0)) | 0, true); return; // fist
            case 3: cpu.mem.setInt32(m.addr, this.round(this.pop()) | 0, true); return;  // fistp
      ...
      case 0xdd: {
        if (isMem) {
          switch (reg) {
            ...
            case 2: cpu.mem.setFloat64(m.addr, this.get(0), true); return;     // fst m64
            case 3: cpu.mem.setFloat64(m.addr, this.pop(), true); return;      // fstp m64
            case 4: return;                                                    // frstor
            case 6: return;                                                    // fnsave
            case 7: cpu.mem.setUint16(m.addr, this.statusWord(), true); return; // fnstsw
          }
      ...
      case 0xdf: {
        if (isMem) {
          switch (reg) {
            ...
            case 2: cpu.mem.setInt16(m.addr, this.round(this.get(0)), true); return;  // fist m16
            case 3: cpu.mem.setInt16(m.addr, this.round(this.pop()), true); return;   // fistp m16
            case 5: {                                                          // fild m64

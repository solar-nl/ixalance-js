...
export class Machine {
  ...
  updateMusic() {
    ...
    if (this.xm) {
      ...
      const { rowCounts, totalRows, restartRows } = this.xm;
      // MIDAS and the browser's XmPlayer both loop at the module restart
      // position. The sound-off clock must do the same: several productions
      // deliberately wait for the position to wrap before returning.
      if (rows >= totalRows) {
        const loopRows = totalRows - restartRows;
        rows = loopRows > 0 ? restartRows + ((rows - restartRows) % loopRows) : 0;
      }
      for (let i = 0; i < rowCounts.length; i++) {
        if (rows < rowCounts[i]) { pos = i; row = rows; break; }
        rows -= rowCounts[i];
        pos = i; row = rowCounts[i] - 1;
      }
    }
    this.mem.setUint8(this.pMustime, pos & 0xff);
    this.mem.setUint8(this.pMustime + 1, row & 0xff);
  ...
  startXm(ptr, len) {
    ...
    let restart = m.getUint16(ptr + 0x42, true);
    ...
    if (restart >= songLength) restart = 0;
    const totalRows = rowCounts.reduce((a, b) => a + b, 0);
    const restartRows = rowCounts.slice(0, restart).reduce((a, b) => a + b, 0);

    this.xm = {
      bytes, speed, bpm, songLength, numPatterns, rowCounts,
      restart, totalRows, restartRows,
    };
    ...
    this.onDebug(`XM: ${len} bytes, ${songLength} orders, ${numPatterns} patterns, `
                 + `speed ${speed}, ${bpm} BPM, ${totalRows} rows`);
    this.onMusic?.(bytes);
  }
